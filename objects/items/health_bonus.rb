...
class HealthBonus < Collectible
  ...
  def initialize(x:0, y:0, destroy_time:(Time.now.to_f + Options::COLLECTIBLE_DESTROY_TIMER))
    super(type:ObjectType::HEALTH_BONUS, x:x, y:y, destroy_time:destroy_time)

    @sprite = GameHandler.get_instance.get_sprite(sprite_id:SpriteID::HEALTH_BONUS)
  end

  def collected(player:)
    player.heal(health:Options::HEALTH_BONUS_HEAL_AMOUNT)
  end
